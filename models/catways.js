...
const catwaysSchema = new Schema(
  {
    ...
    type: {
      type: String,
      enum: ['long', 'short'],
      required: [true, `Catway's type is required (long or short).`],
    },
    state: {
      type: String,
      ...
      required: [true, `Catway's state is required and must be between 2 to 200 character long.`],
      match: [/^\p{L}(?:[\p{L}\p{M}\p{N}\s':;,"()-.!]{0,200}[\p{L}.!])$/u, `Catway's state should be describe within 2 to 200 characters long.`],
    },
  },
  {
    timestamps: true,
